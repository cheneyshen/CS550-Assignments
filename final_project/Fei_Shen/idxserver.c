...
void * thread_backup(void *arg)
{
        ...
        if (-1 == write(cpfile->socket, str_result, strlen(str_result)))
            FUNW("write");

        printf("write:%s", str_result);
        if (-1 == recv(cpfile->socket, fileinfo, sizeof(fileinfo), 0))
            FUNW("recv");
        printf("filesize:%s\n", fileinfo);
        ...
        while( 1 )
        {
            wn = recv(cpfile->socket, str_result, BUFSIZE, 0);
        ...
        if (cpfile->socket > 0 && -1 ==write(cpfile->socket, str_result, strlen(str_result)))
            FUNW("write");
        close(cpfile->socket);
        return ((void *)NULL);
...
void * process_cli(void *arg)
{
    ...
    if (0 == strncasecmp(info->buffer, "regist", 6))
    {
       ...
       if (-1 == write(info->socket, str_result, strlen(str_result)))
        ...
        if (info->bckskt > 0) {
            // Continue to transfer to backup server
            if (-1 == write(info->bckskt, info->buffer, BUFSIZE))
                FUNW("write back server");

            bzero(str_result, BUFSIZE);
            strcpy(str_result, "obtain ");
            strcat(str_result, str_key);
            str_result[strlen(str_result)] = '\0';
            //printf("str_result:%s", str_result);
            if (-1 == write(info->socket, str_result, strlen(str_result)))
                FUNW("write");
            ...
            if (-1 == access(str_result, X_OK))
            {
                int status = mkdir(str_result, S_IRWXU | S_IRWXG | S_IRWXO); //777
                (!status) ? (printf("Directory created %s\n", str_result)) : (printf("Unable to create directory %s\n", str_result));
            }
            ptr = strrchr(str_key,  r);
            strcat(str_result, ptr);
            ...
            while( 1 )
            {
                wn = recv(info->socket, str_result, BUFSIZE, 0);
                fwrite(str_result, 1, wn, fp);
                //printf("wn=%d\n", wn);
                filesize -= wn;
                if (filesize  == 0 )    // failed connection processing
    ...
    else if (0 == strncasecmp(info->buffer, "search", 6) )
    {
        ...
        // search the backup server and write back to info->socket
        if (info->bckskt > 0) {
            // Continue to transfer to backup server
            if (-1 == write(info->bckskt, info->buffer, BUFSIZE))
                FUNW("write back server");
            //printf("buffer:%s", info->buffer);
            if (-1  == read(info->bckskt, str_result, BUFSIZE))
                    FUNW("read");
